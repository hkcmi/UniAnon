...
Browser requests to `/auth/oidc/callback` receive a small handoff page that stores the session token and redirects users back to the UniAnon UI. API clients that do not request HTML receive session JSON.
